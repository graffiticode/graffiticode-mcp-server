...
- **`src/widget/`** - A **native** inline widget that renders items *in the host sandbox* — no iframe. It is a **Claude-only** experience by a deliberate **whitelist**: `toolsForClient()` (in `tools.ts`) wires the MCP Apps resource onto each widget-bearing tool's `_meta` (nested `ui.resourceUri` + legacy `ui/resourceUri`, mimeType `text/html;profile=mcp-app`) **only for verified widget hosts — `isWidgetHost()` = the Claude family (`/claude/i`)**. Every other client (all ChatGPT/OpenAI surfaces, Codex, and any unknown) gets **no widget metadata**, no hydration `_meta`, and no widget resource in `resources/list` — just the tool result's text summary plus its "Open in Graffiticode" link. Whitelisting (not blacklisting OpenAI) is intentional: ChatGPT's consumer app reports a client name the old blacklist missed, so the widget leaked and got stuck "Generating…" on web; the whitelist guarantees a clean, consistent no-UI OpenAI experience for submission. Only `create_item`/`update_item` are non-widget-bearing (they return "generating"); `render_item`/`get_item` carry the widget marker. There is **no** `form_url` / token-bearing render URL, and no iframe.
