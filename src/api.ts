...
// The L0182 language server, which fronts the collective-intelligence service. Survey
// participation goes here rather than through the console: no console mutation accepts a data
// payload (startCodeGeneration is the only write, is LLM-mediated and destructive), and this
// is the same proxy the rendered form calls — one sampler, one pool, one code path, which is
// what makes an agent's participation indistinguishable from a person's.
export const L0182_URL = process.env.GRAFFITICODE_L0182_URL || "https://l0182.graffiticode.org";
...
// ---------------------------------------------------------------------------
// Survey participation (L0182)
// ---------------------------------------------------------------------------

export interface SurveyIdea {
  id: string;
  text: string;
  score?: number;
}

/** What the service says this participant should be looking at now. */
...
/**
 * Call the survey proxy.
 *
 * The two headers are the whole difference between an agent's participation and a person's.
 * The proxy derives `actor.class` from them and refuses to read a class out of the body, so a
 * caller cannot enter a human-only session by claiming to be human.
 */
async function surveyCall(path: string, body: unknown, clientKind?: string): Promise<SurveyFrame> {
  const startedAt = Date.now();
  const headers: Record<string, string> = {
    "Content-Type": "application/json",
    "X-Graffiticode-Client": "mcp",
  };
  if (clientKind) headers["X-Graffiticode-Client-Host"] = clientKind;

  let response: Response;
  try {
    response = await fetch(`${L0182_URL}${path}`, {
      method: "POST",
      headers,
    ...
    recordUpstream(startedAt);
    throw new Error(`survey service unreachable: ${err instanceof Error ? err.message : String(err)}`);
  }
  recordUpstream(startedAt);
  ...
  try {
    ...
    throw new Error(`survey service returned a non-JSON response (${response.status})`);
  }
  if (!response.ok) {
    throw new Error(out?.error?.message || `survey service error ${response.status}`);
  }
  return out as SurveyFrame;
}

/** Start or resume a participation. */
export const openSurvey = (options: {
  session?: string;
  participation?: string;
  participants?: string[];
  items?: SurveyItemRef[];
...
export const answerSurvey = (options: {
  session?: string;
  participation: string;
  participants?: string[];
  items?: SurveyItemRef[];
